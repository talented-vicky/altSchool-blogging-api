    ...
    new localStrategy(
        ...
        async (req, email, password, done) => {
            try{
                ...
                const author = await AuthModel.create({ 
...
passport.use(
    'login', // this string must be the same in passport.authenticate
    new localStrategy(
        {
            usernameField: 'email',
        ...
        async (email, password, done) => {
            try{
                ...
                const validPassword = await author.isValidPassword({ password})
